#' Unified OCR Interface for Multiple Providers
#'
...
#' @param provider Character string. OCR provider to use. One of:
#'   \describe{
...
#' @param api_key Character string. API key for the provider. If NULL (default),
...
ohseer_ocr <- function(file_path,
  ...
  result <- switch(
    ...
    "tensorlake" = {
      # Tensorlake uses max_wait_seconds instead of timeout
      tensorlake_ocr(
        file_path = file_path,
        pages = pages,
    ...
    "mistral" = {
      # Mistral uses 'input' parameter and 'timeout'
      mistral_ocr(
        input = file_path,
        timeout = timeout,
        api_key = api_key,
    ...
    "claude" = {
      # Claude doesn't have a timeout parameter (uses max_tokens instead)
      # and uses file_path
  ...
  if (extract_pages) {
    extracted_pages <- switch(
      provider,
      "tensorlake" = tensorlake_extract_pages(result, pages = pages),
      "mistral" = mistral_extract_pages(result, pages = pages),
      "claude" = claude_extract_pages(result, pages = pages),
      stop("Unknown provider: ", provider, call. = FALSE)
    )
